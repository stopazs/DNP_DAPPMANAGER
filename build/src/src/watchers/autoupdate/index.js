...
const findLatestVersion = (store, packagename) => {
    ...
    return latestPackage ? latestPackage.manifest : null;
};
        ...
        r2]).then(async ([packageList, store]) => {
            const upgrades = Promise.all(packageList.map(async (installedPackage) => {
                ...
                if (autoUpdate === true) {
                    ...
                    if (latestVersion && semver.gt(latestVersion.version, installedPackage.version)) {
                        console.log(`package ${installedPackage.name} needs update from ${installedPackage.version}->${latestVersion.version}`);
                        return (`${installedPackage.name}@${latestVersion.hash}`);
                    } else {
                        console.log(`no update. local=${installedPackage.version}, store=${latestVersion.version}`);
                    }
                } else {
            ...
            upgrades.then((upgradeList) => {
                ...
                if (toUpgrade.length > 0) {
                    ...
                    calls.installPackage({ id: item }).then((res) => { logs.info(`installed package ${item}`) });
